...
| Same-session rewind | ✅ механизм, 🟡 усиленный final evidence | Abort текущего хода, остановка активных workers, `navigateTree(..., summarize:false)` в том же JSONL, возврат prompt/images, abandoned leaf и Return. Advanced gate теперь отдельно проверяет session/leaf identity, invalid target и aliasing | Rewind намеренно не откатывает workspace-файлы; destructive code restore должен быть отдельным явным выбором. Path attachments возвращаются в тексте prompt, но не как отдельные chips; новый advanced fingerprint должен пройти повторно |
