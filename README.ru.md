...
- **Долгие фоновые задачи** — статус и управление в top bar, heartbeat, защита от idle/eviction и подтверждение выхода для многочасовой работы
- **Управление сессиями** — рабочие пространства, поиск, rewind «изменить и повторить» внутри той же сессии с подтверждённым откатом файлов, явный форк, закрепление, архив, группы
- **AI-названия сессий** — Recommended-профиль генерирует короткое имя отдельным Pi-вызовом с отключённым thinking и не перезаписывает ручное название
- **Семейства иконок macOS 26/27** — Liquid Glass, Aurora и Graphite с живым переключением Dock/UI glyphs и режимом следования основной теме
...
- **Фоновая работа** — очередь субагентов видна над composer; поддерживается изоляция через worktree
- **Адаптивный workflow** — наблюдаемые project gates, независимый evaluator, ограниченный repair-loop и усиленный always-on Ponytail
...
Установка, обновление, удаление и включение отдельных ресурсов выполняются
транзакционно: приложение создаёт снимок затронутых scope, восстанавливает
проверенные harness-overlays, запускает настоящий runtime Pi и автоматически
откатывается при ошибке загрузки или конфликте capability. Подробнее:
[жизненный цикл extensions](docs/EXTENSION-LIFECYCLE.md).
...
(харнесс, память/производительность, экосистема, дизайн-система). Реализованный
контракт и честные остаточные gaps описаны в
**[Harness v3](docs/HARNESS-REDESIGN-2026-07-18.md)** и
**[аудите покрытия](docs/HARNESS-GAP-AUDIT-RU-2026-07-18.md)**. Открытые задачи —
