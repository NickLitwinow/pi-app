...
- **Организация сессий**: группы/папки внутри проекта, pin/архив; закрепляемые
  ...
  растягиваемый сайдбар; управление проектами (убрать из сайдбара / открыть в Finder).
- **Параллельная работа с сессиями**: пока агент занят долгой задачей, можно
  временно открыть другую сессию — она показывается мгновенно (read-only), агент
  продолжает в фоне, разрешения остаются отвечаемыми, кнопка «Вернуться к активной».
  Полная история (включая до-компакции) читается из файла активной ветки.
- **Экономная память**: история фоновых проектов выгружается и восстанавливается
  из файла; выводы инструментов ограничиваются, старые — выгружаются с пометкой.
- **Create PR/MR** под любой аккаунт: GitHub (`gh`), GitLab (`glab`) или открытие
  страницы создания запроса в браузере — по origin рабочего проекта и локальной авторизации.
- **Самообновление**: кнопка «Обновления» в сайдбаре — проверка последней версии
  на GitHub, локальный ребилд из исходников (git pull → npm → tauri build) и замена
  приложения с перезапуском.

Все подтверждения/диалоги идут через нативный Tauri-плагин (в WKWebView `window.confirm`
не работает), поэтому кнопки с подтверждением надёжны.
